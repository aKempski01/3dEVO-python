...
class AVGPlotDisplayer(QtWidgets.QWidget):
    ...
    def __update_matrix(self):
        self.game_matrix = np.zeros([self.__logic_handler.param_handler.population_length]*self.__logic_handler.param_handler.num_dim + [self.__logic_handler.param_handler.num_phenotypes])
        for i in range(self.first_epoch, self.last_epoch+1):

            self.game_matrix += self.__logic_handler.get_array(i)
        self.game_matrix /= np.max(self.game_matrix)

    def __slider_released_signal(self):
        self.first_epoch, self.last_epoch = self.slider.value()
        self.__update_matrix()
        self.__update_plot()
